...
#It seems to load if you press the butto a few times, fine for presenetation
...
def get_daily_totals(user_id, target_date):
    ...
    for e in entries:
        ...
        #fixed the calories adding not working
...
#Added dyslexia font route update
@app.route("/toggle-dyslexia")
def toggle_dyslexia():
    session["dyslexia_font"] = not session.get("dyslexia_font", False)
...
def logout():
    session.clear()
    flash("You have been logged out.", "info") #for logout
    return redirect(url_for("login"))
...
def subscriber_home():
    ...
    # build 7day calorie data for the little chart on the dashboard
    chart_labels = []
    chart_calories = []
...
def professional_home():
    ...
    # beach subscriber with their daily totals guideline for the dashboard
    subscriber_data = []
...
def view_client(subscriber_id):
    ...
    for i in range(6, -1, -1):
        d = today - timedelta(days=i) #fixed
        chart_labels.append(d.strftime("%a %d"))
...
def set_guidelines(subscriber_id):
    ...
    if request.method == "POST":
        try:
            # using  trick so empty strings dont become 0.0 in the db
